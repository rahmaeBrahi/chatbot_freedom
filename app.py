...
try:
    ...
    # Plain WSGI app fallback to render the traceback directly on screen
    def app(environ, start_response):
        start_response('500 Internal Server Error', [('Content-Type', 'text/html')])
        html = f"""
        <html>
            <head><title>Import Error Traceback</title></head>
            <body style="font-family: monospace; padding: 20px; background: #fafafa; color: #333;">
                <h1 style="color: #d32f2f;">Import Error during Vercel Initialization</h1>
                <hr>
                <pre style="background: #f1f1f1; padding: 15px; border-radius: 5px; border: 1px solid #ccc; overflow-x: auto;">{tb}</pre>
            </body>
        </html>
        """
